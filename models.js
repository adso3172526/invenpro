// InvenPro — Modelos y Servicios (clases ES6)
// Capa de dominio: encapsula entidades y operaciones de base de datos.
...
class Producto {
  ...
  get initials() {
    return this.nombre
      ? this.nombre.split(" ").map(w => w[0]).slice(0, 2).join("").toUpperCase()
      : "?";
  }
...
class Factura {
  ...
  get cantidadItems() {
    return this.items.reduce((sum, it) => sum + (it.q || 0), 0);
  }
}
...
class AuthService {
  async login(usuario, pass) {
    ...
    const hashed = await window.hashPass(pass);
...
// Exportar clases al scope global
